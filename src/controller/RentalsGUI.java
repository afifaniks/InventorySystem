...
public class RentalsGUI implements Initializable {
    ...
    void btnProceedAction(ActionEvent event) {
        if (startTransaction) {
            startTransaction = false; //Resetting TransactionGUI Value
            ...
            TransactionGUI.rentalId = lblId.getText();
            TransactionGUI.itemID = Integer.valueOf(txtItemId.getText());
            TransactionGUI.rentalReturnDate = txtReturnDate.getValue();
            TransactionGUI.rentOrSale = false;
            ...
            try {
                ...
                if(stock == 0) {
                    new PromptDialogGUI("Insufficient Stock!", "This item is not currently available for renting.");
                    return;
                }

                TransactionGUI.stock = stock; //because its rental
