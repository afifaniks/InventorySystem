...
public class AccountGUI implements Initializable {
    ...
    @FXML
    private TableView<main.Account> tblRecent;

    @FXML
    private TableColumn<main.Account, String> customer;

    @FXML
    private TableColumn<main.Account, Integer> accID;

    @FXML
    private TableColumn<main.Account, String> accName;

    @FXML
    private TableColumn<main.Account, String> payMethod;
    ...
    public static ObservableList<main.Account> accountList;
    public static ObservableList<main.Account> tempList;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        lblProcessedBy.setText(LogInGUI.loggerUsername);
    ...
    void btnSearchAction(ActionEvent event) {
        if (searchDone) {
            ...
            ObservableList<main.Account> searchResult = FXCollections.observableArrayList(); //list to hold search result
            try {
                PreparedStatement ps = con.prepareStatement(SQL);
                ps.setString(1, "%" + txtSearch.getText() +"%");
                ResultSet accountResultSet = ps.executeQuery();
                while (accountResultSet.next()) {
                    searchResult.add(new main.Account(accountResultSet.getInt(3),
    ...
    void reloadAll(ActionEvent event) {
        ObservableList<main.Account> accountListByUser = FXCollections.observableArrayList();
        ...
        try {
            ...
            PreparedStatement getAccountList = connection.prepareStatement("SELECT  customers.firstName, customers.lastName, accounts.acccountID, accounts.accountName, accounts.paymethod " +
                    ...
                    + LogInGUI.loggerUsername+"' AND Customers_customerID = customerID");

            ResultSet accountResultSet = getAccountList.executeQuery();

            while(accountResultSet.next()) {
                accountListByUser.add(new main.Account(accountResultSet.getInt(3),
                        accountResultSet.getString(1) + " " + accountResultSet.getString(2),
                        accountResultSet.getString(4),
                        accountResultSet.getString(5)));
            }
    ...
    void btnAddAction(ActionEvent event) {
        ...
        if(!flag){
            ...
            try {
                ...
                ps.setString(4, LogInGUI.loggerUsername);
                ...
                new PromptDialogGUI("Operation Successful!", "Account successfully created and stored into database.");

                initiate();
            } catch (SQLException e) {
                new PromptDialogGUI("Error!", "Invalid Argument.");
            }
        }
    }
